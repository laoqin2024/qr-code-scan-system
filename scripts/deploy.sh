...
show_summary() {
    ...
    if [ "$USE_PM2" = "y" ]; then
        ...
        echo "    pm2 logs scan-code-backend    # 查看实时日志"
        echo "    pm2 logs --lines 100          # 查看最近100行日志"
        echo "    pm2 restart scan-code-backend # 重启服务"
        echo "    pm2 stop scan-code-backend    # 停止服务"
        echo "    pm2 delete scan-code-backend  # 删除进程"
        echo "    pm2 monit                     # 实时监控"
        echo ""
        echo "  PM2 开机自启:"
        echo "    pm2 save                      # 保存当前进程列表"
        echo "    pm2 startup                   # 生成开机启动脚本"
        echo "    pm2 unstartup                 # 禁用开机启动"
        echo ""
        echo "  PM2 日志管理:"
        echo "    pm2 flush                     # 清空日志"
        ...
        echo "  启动服务: cd $INSTALL_DIR/backend && nohup npm start > ../logs/backend.log 2>&1 & echo \$! > ../backend.pid"
    fi
    echo ""
    echo "📚 文档:"
